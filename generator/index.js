...
module.exports = (api, options, rootOptions, invoking) => {
  const routeRootDir = './src/router'
  const viewsRootDir = './src/views'
  const routeRootFile = './src/router/index.js'
  const templatesRoot = './templates'
  const moduleDir = path.join(routeRootDir, `${options.moduleName}`)
  ...
  // Verify routeRootFile exists, if not Render
  if (!fs.existsSync(`${routeRootFile}`)) {
    api.render({
      [`${routeRootFile}`]: `${templatesRoot}/index.js`
    })
